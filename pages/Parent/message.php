...
if (isset($_GET['action'])) {
  ...
  try {
    ...
    if ($action === 'thread') {
      ...
      $upd->execute([':with' => $withId, ':me' => $parentId]);

      // Fetch last 200 messages between parent and staff
      $stmt = $pdo->prepare('
        ...
        LIMIT 200
      ');
      $stmt->execute([':me' => $parentId, ':with' => $withId]);
    ...
    if ($action === 'send' && $_SERVER['REQUEST_METHOD'] === 'POST') {
      ...
      $ins->execute([':sender' => $parentId, ':recipient' => $to, ':body' => $body]);
    ...
    if ($action === 'edit' && $_SERVER['REQUEST_METHOD'] === 'POST') {
      ...
      $upd->execute([':body' => $body, ':id' => $id, ':me' => $parentId]);
      if ($upd->rowCount() === 0) {
...
$pageTitle = 'Messages - Parent - SDMS';
...
<script>
  ...
  function renderMessages(payload) {
    ...
    for (const m of messages) {
      const mine = parseInt(m.sender_user_id) === parentId;
