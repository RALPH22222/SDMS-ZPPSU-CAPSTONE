<?php
// Parent Notifications Management & Inbox
...
// Ensure we have a parent user id (fallback to first parent if session empty)
$parentId = (int)($_SESSION['user_id'] ?? 0);
if ($parentId === 0) {
  try {
    $stmtParent = $pdo->query('SELECT id FROM users WHERE role_id = 4 AND is_active = 1 ORDER BY id ASC LIMIT 1');
    $fallbackId = (int)($stmtParent->fetchColumn() ?: 0);
    if ($fallbackId > 0) { $parentId = $fallbackId; }
...
// API Endpoints (list, mark_read, mark_all_read, delete, methods, stats)
if (isset($_GET['action'])) {
  ...
  try {
    ...
    if ($action === 'mark_read' && $_SERVER['REQUEST_METHOD'] === 'POST') {
      ...
      $upd->execute([':id' => $id, ':uid' => $parentId]);
    ...
    if ($action === 'mark_all_read' && $_SERVER['REQUEST_METHOD'] === 'POST') {
      ...
      if ($methodId > 0) { $where .= ' AND method_id = :mid'; $params[':mid'] = $methodId; }
    ...
    if ($action === 'delete' && $_SERVER['REQUEST_METHOD'] === 'POST') {
      ...
      $del->execute([':id' => $id, ':uid' => $parentId]);
    ...
    if ($action === 'methods') {
      ...
      // Also include unread counts per method for this user
...
// Page render
$pageTitle = 'Notifications - Parent - SDMS';
require_once __DIR__ . '/../../components/parent-head.php';
?>
...
<script>
  ...
  document.getElementById('parentSidebarToggle')?.addEventListener('click', () => {
    const el = document.getElementById('parentSidebar');
    if (!el) return;
    const hidden = el.classList.contains('-translate-x-full');
    if (hidden) el.classList.remove('-translate-x-full'); else el.classList.add('-translate-x-full');
  });

  const parentId = <?php echo (int)$parentId; ?>;
  ...
  function loadSettings() {
    try {
      setSoundEl.checked = localStorage.getItem('parent_notif_sound') === '1';
      setDesktopEl.checked = localStorage.getItem('parent_notif_desktop') === '1';
    } catch (e) {}
  }
  function saveSettings() {
    try {
      localStorage.setItem('parent_notif_sound', setSoundEl.checked ? '1' : '0');
      localStorage.setItem('parent_notif_desktop', setDesktopEl.checked ? '1' : '0');
    } catch (e) {}
  }
...
<?php require_once __DIR__ . '/../../components/parent-footer.php'; ?>