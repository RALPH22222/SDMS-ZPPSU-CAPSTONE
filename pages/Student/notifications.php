<?php
// Student Notifications Management & Inbox
...
// Auth guard: must be logged in and role = Student (2)
// if (!isset($_SESSION['user_id']) || !isset($_SESSION['role_id']) || (int)$_SESSION['role_id'] !== 2) {
//   header('Location: /SDMS/pages/Auth/login.php');
//   exit;
// }

// Ensure we have a student user id (fallback to first student if session empty)
$studentId = (int)($_SESSION['user_id'] ?? 0);
if ($studentId === 0) {
  try {
    $stmtStudent = $pdo->query('SELECT id FROM users WHERE role_id = 2 AND is_active = 1 ORDER BY id ASC LIMIT 1');
    $fallbackId = (int)($stmtStudent->fetchColumn() ?: 0);
    if ($fallbackId > 0) { $studentId = $fallbackId; }
...
if (isset($_GET['action'])) {
  ...
  try {
    ...
    if ($action === 'mark_read' && $_SERVER['REQUEST_METHOD'] === 'POST') {
      ...
      $upd->execute([':id' => $id, ':uid' => $studentId]);
    ...
    if ($action === 'delete' && $_SERVER['REQUEST_METHOD'] === 'POST') {
      ...
      $del->execute([':id' => $id, ':uid' => $studentId]);
...
// Page render
$pageTitle = 'Notifications - Student - SDMS';
...
<script>
  ...
  function loadSettings() {
    try {
      setSoundEl.checked = localStorage.getItem('student_notif_sound') === '1';
      setDesktopEl.checked = localStorage.getItem('student_notif_desktop') === '1';
    } catch (e) {}
  }
  function saveSettings() {
    try {
      localStorage.setItem('student_notif_sound', setSoundEl.checked ? '1' : '0');
      localStorage.setItem('student_notif_desktop', setDesktopEl.checked ? '1' : '0');
    } catch (e) {}
  }
