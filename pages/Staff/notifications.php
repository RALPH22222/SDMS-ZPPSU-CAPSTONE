<?php
// Staff Notifications Management & Inbox
...
// Auth guard: must be logged in and role = Staff (4)
// if (!isset($_SESSION['user_id']) || !isset($_SESSION['role_id']) || (int)$_SESSION['role_id'] !== 4) {
//   header('Location: /SDMS/pages/Auth/login.php');
//   exit;
// }

// Ensure we have a Staff user id (fallback to first Staff if session empty)
$StaffId = (int)($_SESSION['user_id'] ?? 0);
if ($StaffId === 0) {
  try {
    $stmtStaff = $pdo->query('SELECT id FROM users WHERE role_id = 4 AND is_active = 1 ORDER BY id ASC LIMIT 1');
    $fallbackId = (int)($stmtStaff->fetchColumn() ?: 0);
    if ($fallbackId > 0) { $StaffId = $fallbackId; }
...
// Page render
$pageTitle = 'Notifications - Staff - SDMS';
...
<script>
  ...
  document.getElementById('StaffSidebarToggle')?.addEventListener('click', () => {
    const el = document.getElementById('StaffSidebar');
    if (!el) return;
  ...
  function loadSettings() {
    try {
      setSoundEl.checked = localStorage.getItem('Staff_notif_sound') === '1';
      setDesktopEl.checked = localStorage.getItem('Staff_notif_desktop') === '1';
    } catch (e) {}
  }
  function saveSettings() {
    try {
      localStorage.setItem('Staff_notif_sound', setSoundEl.checked ? '1' : '0');
      localStorage.setItem('Staff_notif_desktop', setDesktopEl.checked ? '1' : '0');
    } catch (e) {}
  }
